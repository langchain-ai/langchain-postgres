...
def _get_embedding_collection_store(vector_dimension: Optional[int] = None) -> Any:
    ...
    class CollectionStore(Base):
        ...
        def get_by_name(
            ...
            # FIXME return session.query(cls).filter(cls.name == name).first()  # type: ignore
            return session.execute(
                select(cls).filter(cls.name == name)).scalars().first()
...
class PGVector(VectorStore):
    ...
    * A Connection object has to be provided explicitly. Connections will not be
    ...
    async def acreate_tables_if_not_exists(self) -> None:
        if isinstance(self._bind, sqlalchemy.ext.asyncio.engine.AsyncConnection):
            await self._bind.run_sync(Base.metadata.create_all)
        else:
            async with self._bind.begin() as conn:  # FIXME: session.run_sync existe
    ...
    # async def aadd_embeddings( REFAIRE
    #         self,
    #         texts: Iterable[str],
    #         embeddings: List[List[float]],
    #         metadatas: Optional[List[dict]] = None,
    ...
    #             lambda sync_session: sync_session.bulk_save_objects(documents))
    #
    #     return ids
    ...
    # async def __aquery_collection(  # FIXME
    #         self,
    #         session: AsyncSession,
    #         embedding: List[float],
    #         k: int = 4,
    ...
    #
    #     _type = self.EmbeddingStore
    #     stmt = (select(self.EmbeddingStore,
    ...
    # async def aadd_documents(  # FIXME: remove
    ...
    # def _build_sessionmaker(self) -> sessionmaker:
    #     return sessionmaker(bind=self._bind)
    #
    # def _build_async_sessionmaker(self) -> sessionmaker:
    #     return async_sessionmaker(
    #         bind=self._bind,
    #         sync_session_class=self._session_maker)
    #
    # @contextlib.contextmanager
    # def _session_maker(self) -> Generator[Session, None, None]:
    #     """Create a context manager for the session, bind to _conn string."""
    #     session = self._session_maker()
    #     session.begin()  # FIXME: sans ?
    #     yield session
    #     session.commit()  # FIXME
    #
    # @contextlib.asynccontextmanager
    # async def _amake_session(self) -> Generator[AsyncSession, None, None]:
    #     """Create a context manager for the session, bind to _conn string."""
    #     async_session: AsyncSession = self._session_maker()
    #     await async_session.begin()
    #     yield async_session
    #     await async_session.commit()  # FIXME
    #
